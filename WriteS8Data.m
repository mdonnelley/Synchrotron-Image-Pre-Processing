    ...
    if date_list(i).isdir,
        ...
        for j = 1:length(sample_list),
            ...
            if sample_list(j).isdir,
                ...
                run_dir = sample_list(j).name;
                run_list = dir(fullfile(date_dir,sample_dir,run_dir,[expt.naming.folder_prefix,'*']));
                ...
                for k = 1:length(run_list)
                    ...
                    if run_list(k).isdir,
                        ...
                        image_dir = run_list(k).name;

                        %% Check if there are flat files present
                        image_list = dir(fullfile(date_dir,sample_dir,run_dir,image_dir,['*',expt.naming.flat_prefix,'*',expt.naming.type]));
                        if ~isempty(image_list),
                            data{row,6} = fullfile(sample_dir,run_dir,image_dir);
                            outfile = regexp(image_list(i).name,'_|\.','split');
                            data{row,7} = [outfile{1},'_'];
                        end

                        %% Check if there are dark files present
                        image_list = dir(fullfile(date_dir,sample_dir,run_dir,image_dir,['*',expt.naming.dark_prefix,'*',expt.naming.type]));
                        if ~isempty(image_list),
                            data{row,11} = fullfile(sample_dir,run_dir,image_dir);
                            outfile = regexp(image_list(i).name,'_|\.','split');
                            data{row,12} = [outfile{1},'_'];
                        end

                        %% Get the list of all experiment files with the expt.naming.file_prefix
                        image_list = dir(fullfile(date_dir,sample_dir,run_dir,image_dir,[expt.naming.file_prefix,'*',expt.naming.run_prefix,'*',expt.naming.type]));
                        if ~isempty(image_list),
                            ...
                            for m = 1:runs,
                                ...
                                imagepath = fullfile(sample_dir,'/',run_dir,image_dir,'/');
                                imagestart = sprintf('%s%.2d%s%.2d',expt.naming.file_prefix,ID,expt.naming.run_prefix,m);
                                sub_run_list = dir(fullfile(date_dir,imagepath,[imagestart,'*',expt.naming.type]));

                                if ~isempty(sub_run_list),

                                    outfile = regexp(sub_run_list(1).name,'_|\.','split');
